...
const (
	...
	playerInfoBackReferenceBias = Symbols
)
...
func buildCumulativeRowFromCounts(counts [Symbols]uint32) [Symbols]uint32 {
	...
	for i, v := range counts {
		accum += uint64(v)
		row[i] = uint32((accum * cumulativeTotal) / total)
	}
	return row
}
...
func buildPairedCumulativeRows(tables *Tables, raw *[Rows][Symbols]uint32, row int) {
	...
	for i, v := range pair {
		accum += uint64(v)
		cum := uint32((accum * cumulativeTotal) / total)
		if i < Symbols {
...
func NewTables(data []byte) (*Tables, error) {
	...
	raw[RowIndex(SVCPlayerInfoBackReference)][1] += playerInfoBackReferenceBias
	...
	for row := RowIndex(SVCVoiceData); row < len(tables.Cumulative); row++ {
		tables.Cumulative[row] = buildCumulativeRowFromCounts(raw[row])
	}
